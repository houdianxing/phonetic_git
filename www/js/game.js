(function() {
	...
	var panelObj, backObj, numObj, playObj, finishObj; //
	var mx, my; //鼠标的横纵坐标值。
	var words;
//	var word;
	var letters = [];
	 ...
	 */
	bdc.startgame = function(ws, scope, ppspeed) {
		letters = [];
		_index = 0;
		bdc.init( ws, scope, ppspeed);
		lastframetime = Date.now();
		bdc.gameLoop();
	}
	bdc.init = function(ws, scope, ppspeed) {
		qipap_speed = ppspeed;
		pagescope = scope;
		letters = [];
		var h = screen.height;
		...
		for(var i = 0; i < words.length; i++) {
			letters.push(words[i].en.replaceAll("/",""));
		}
...
//		explianObj = new explianObject();
		...
		playWordAudio(words[_index].audio_0);
	...
	bdc.setData = function() {
		_index = _index + 1;
//		letters = [];
//		for(var i = _index; i < words.length; i++) {
//		   letters.push(words[i].en.replaceAll("/",""));
//		}
		...
		if(len1 < 20) {
			...
			for(var i = 0; i < paopOb.num; i++) {
				if(paopOb.alive[i] && paopOb.grow[i]) {
					...
					if(len < 30) {
						if(letters[i] == words[_index].en.replaceAll("/","")) {
...
//		}
	...
	function computePaopao() { //计算屏幕上的气泡数量
		...
		for(var i = 0; i < paopOb.num; i++) {
			...
			if(paopOb.alive[i] && i<_index){
				paopOb.alive[i] = false;
			}
		}
		if(count < paopOb.num-_index) {
			bornPaopao(); //出生一个气泡
			return false;
		}
